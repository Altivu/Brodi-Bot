      ...
      if (rows.length) {
        ...
        if (args.length > 0) {
          ...
          if (searchString.split(" ")[0].toLocaleLowerCase() === "tierlist") {
            ...
            let validKarts = obj.filter(kart => kart["Role (Item Karts Only)"]).sort((a, b) => roleSortKeys[a["Role (Item Karts Only)"].split("-")[0].trim()] - roleSortKeys[b["Role (Item Karts Only)"].split("-")[0].trim()] || parseFloat(a["Role (Item Karts Only)"].split("-")[1].trim().split(" ")[1].trim()) - parseFloat(b["Role (Item Karts Only)"].split("-")[1].trim().split(" ")[1].trim()) || a["Name"].localeCompare(b["Name"]));

            let releasedValidKarts = validKarts.filter(kart => kart["Released"] === "TRUE");
            let unreleasedValidKarts = validKarts.filter(kart => kart["Released"] !== "TRUE");
            ...
            if (searchString.split(" ")[1] !== "released") {
              ...
              Object.keys(masterTierObj).forEach(role => Object.keys(masterTierObj[role]).forEach(tier => masterTierObj[role][tier].push("-----")));
        ...
        if (kart) {
          ...
          if (kart["Role (Item Karts Only)"]) {
            embed.addFields({
              name: `Role ${kart["Released"] === "FALSE" ? "(Theoretical)" : ""}`,
