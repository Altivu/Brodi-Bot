...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('records')
    .setDescription(
      '(INVERSE MEMBERS ONLY) Get or update a record on the Inverse time sheet.'
    )
    ...
    .addSubcommand(subcommand =>
      subcommand
        ...
        .addStringOption(option =>
          option
            .setName('time')
            .setDescription('Record time (expected format: XX.XX.XX).')
            .setRequired(true)
        )
    ),
  async execute(client, interaction, args, embed, auth) {
    ...
    try {
      ...
      if (
        timeToUpdateWith === '0' ||
        timeToUpdateWith === '00.00' ||
        timeToUpdateWith === '00.00.00'
      ) {
        timeToUpdateWith = '';
      } else {
      ...
      const filter = i => {
        return i?.message?.interaction?.id === interaction.id && i?.user?.id === user?.id;
      }
        ...
        if (i.customId === 'confirm') {
          ...
          if (timeToUpdateWith) {
            resultEmbed.setDescription(
              `${trackNameToSearch} updated with record time of ${timeToUpdateWith}.`
            );
          } else {
            resultEmbed.setDescription(`${trackNameToSearch} record time cleared.`);
          }

          resultEmbed.setFooter(`${previousFooterText} + ${Date.now() - now} ms`);
        }
        else {
      ...
      const row = new MessageActionRow()
        ...
        .addFields({
          ...
          value: timeToUpdateWith || "<<blank>>"
        })
        .addFields({
          name: "Confirmation",
          value: `Confirm that the above information is correct to update the time sheet.
          
This process will automatically cancel if no response is provided within ${+parseFloat((UPDATE_TIME_LIMIT / 1000).toFixed(2))} seconds.`
        });
